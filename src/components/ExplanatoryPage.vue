<script setup>
  ...
  const getJudgeImage = (name) => {
    return new URL(`../assets/images/judges/${name}.png`, import.meta.url).href
  }
</script>
...
<template>
  <div class="pt-10">
    ...
    <div class="grid grid-cols-12 lg:p-2">
        <div v-for="(portfolio,i) in shark" :key="i" class="relative col-span-12 lg:col-span-6 xl:col-span-4 border-2 border-b border-gray-200 rounded-lg mx-1 md:mx-14 lg:mx-4 my-10">
          <img class="absolute inset-x-0.5 -top-14 mx-auto w-28 rounded-full border border-b border-gray-200" :src="getJudgeImage(portfolio[0])">
          <p class="mx-auto mt-20 mb-4 text-xl text-center font-semibold">{{ portfolio[0].toUpperCase()}}</p>
          <div class="mx-4">
            <div class="flex border border-b border-gray-200 rounded p-2 my-4">
              <p class="grow"><img src="../assets/images/rupee.png" alt="Rupee icon"></p>
              <p class="grow">Total money invested</p>
              <p class="grow">{{ portfolio[1][0].total_investment }}</p>
            </div>
            <div class="flex border border-b border-gray-200 rounded p-2 my-4">
              <p class="grow"><img src="../assets/images/deal.png" alt="Handshake icon"></p>
              <p class="grow">Number of deals made</p>
              <p class="grow">{{ portfolio[1][0].number_of_deals }}</p>
            </div>
            <div class="border border-b border-gray-200 rounded p-2 my-4">
              <p class="mx-2 text-center">Industry Investment Mix</p>
              <div class="my-2 h-[50vh]"></div>
            </div>
          </div>
        </div>
    </div>
